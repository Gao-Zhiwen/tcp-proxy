...
public class SQLCacheCmd extends SQLAnnotationCmd {
    private static final Logger LOGGER = LoggerFactory.getLogger(SQLCacheCmd.class);
    public static final SQLCacheCmd INSTANCE = new SQLCacheCmd();
    ...
    public boolean procssSQL(MycatSession session) throws IOException {
        ...
        // 放入sql语句
        session.getSessionAttrMap().put(SessionKeyEnum.SESSION_KEY_CACHE_SQL_STR.getKey(), session.sqlContext.getRealSQL(0));
        ...
        SeqContextList seqcontext = (SeqContextList) session.getSessionAttrMap().get(SessionKeyEnum.SESSION_KEY_CACHE_MYCAT_CHAIN_SEQ.getKey());
        ...
        try {
			...
			LOGGER.error("error to handle sqlCache", e);
        } finally {
            seqcontext.clear();
        }
    ...
    public boolean onFrontWriteFinished(MycatSession session) throws IOException {
        LOGGER.debug("========================> SQLCacheCmd onFrontWriteFinished");
